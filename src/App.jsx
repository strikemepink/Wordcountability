...
function SignIn({onPrivacy}){
  ...
  // Detect if the app is running as an installed PWA on iPhone home screen
  const isStandalone=window.matchMedia("(display-mode: standalone)").matches||window.navigator.standalone===true;

  async function handleGoogle(){
    // Only runs in Safari/browser — never attempted in PWA mode
  ...
  return(
    <div className="leopard leopard-setup" style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px",fontFamily:"'Outfit',sans-serif",color:"#fff"}}>
      ...
      <div style={{maxWidth:380,width:"100%",display:"flex",flexDirection:"column",alignItems:"center",gap:24,textAlign:"center"}}>
        ...
        {/* PWA mode on iPhone — show instructions instead of Google button */}
        {isStandalone?(
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:16,background:"rgba(255,255,255,0.1)",borderRadius:16,padding:"24px 20px",width:"100%"}}>
            <div style={{fontSize:16,fontWeight:700,lineHeight:1.6}}>
              To sign in on iPhone:<br/>
              Open Wordcountability in Safari, sign in with Google there, then hit the three dots and "Add to Home Screen."
            </div>
            <a
              href={APP_URL}
              target="_blank"
              rel="noopener noreferrer"
              style={{background:LF.hotpink,color:"#fff",fontWeight:800,fontSize:16,borderRadius:12,padding:"12px 28px",textDecoration:"none",display:"inline-block"}}
            >
              Open in Safari →
            </a>
            <div style={{fontSize:13,color:"#ffffffcc"}}>You'll only need to do this once.</div>
          </div>
        ):(
          /* Normal browser/Safari — show Google sign-in button as usual */
          <>
            <button className="btn-google" onClick={handleGoogle} disabled={loading}>
              <svg width="20" height="20" viewBox="0 0 48 48">
                ...
                <path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.18 1.48-4.97 2.35-8.16 2.35-6.26 0-11.57-4.22-13.47-9.91l7.98-6.19C6.51 42.62 14.62 48 24 48z"/>
              </svg>
